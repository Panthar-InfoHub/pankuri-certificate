"use server"
import { CompleteMultipartUploadCommand, CreateMultipartUploadCommand, GetObjectCommand, PutObjectCommand, UploadPartCommand } from "@aws-sdk/client-s3";
 ...
 */
export async function getVideoDownloadUrl(storageKey) {
    try {

        const command = new GetObjectCommand({
            Bucket: process.env.DO_PROCESSED_BUCKET,
            Key: storageKey,
            ResponseContentDisposition: `attachment; filename="${storageKey}.mp4"`,
        });
