...
export async function getAllActivePlans({ plan_type = "", subscription_type = "" }) {
    try {
        const params = {};
        if (plan_type) params.type = plan_type;
...
export async function updatePlan(id, data) {
    try {
        ...
        console.debug("Update plan response ==> ", res.data);
        return { success: true, data: res.data.data }
    } catch (error) {
        console.error("Error updating plan:", error.response?.data);
        return { success: false, error: error.response?.data?.message || "update plan failed" }
    }
}
