...
export async function createCourseFaq(payload) {
    try {
        const headers = await getAuthHeaders()
        const res = await axios.post(`${BASE_URL}/faqs/admin/course`, payload, { headers })
        return { success: true, data: res.data.data }
    } catch (error) {
        return { success: false, error: error.response?.data?.message || "createCourseFaq failed" }
    }
}

export async function updateCourseFaq(faqId, payload) {
    try {
        const headers = await getAuthHeaders()
        const res = await axios.patch(`${BASE_URL}/faqs/admin/course/${faqId}`, payload, { headers })
        return { success: true, data: res.data.data }
    } catch (error) {
        return { success: false, error: error.response?.data?.message || "updateCourseFaq failed" }
    }
}
