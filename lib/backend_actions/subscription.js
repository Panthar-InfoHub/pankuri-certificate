...
// ───────────────────────────────────────────────────────────────────────────────
// Admin: Grant Manual Subscription
export async function grantManualSubscription(payload) {
    try {
        const headers = await getAuthHeaders()
        const res = await axios.post(`${BASE_URL}/subscriptions/admin/grant-manual`, payload, { headers })
        return { success: true, data: res.data.data }
    } catch (error) {
        console.error("grantManualSubscription error:", error.response?.data || error.message)
        return { success: false, error: error.response?.data?.message || "grantManualSubscription failed" }
    }
}
